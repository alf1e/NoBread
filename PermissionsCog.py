...
class PermissionsCog(ModuleCog):
    ...
    def create_permissions(self, permission_name: str) -> str:
        ...
        if self.cursor.execute("SELECT permission_name FROM permissions WHERE permission_name = ?", (permission_name,)).fetchone():
    ...
    def delete_permissions(self, permission_name: str) -> None:
        ...
        self.cursor.execute("DELETE FROM permissions WHERE permission_name = ?", (permission_name,))
        self.connection.commit()
    ...
    def add_user(self, permission_name: str, user_id: int) -> None:
        ...
        users = self.cursor.execute("SELECT users FROM permissions WHERE permission_name = ?", (permission_name,)).fetchone()
        if users is None:
            users = []
        else:
            users = users[0].split(",")
        ...
        users = ",".join(users)
        self.cursor.execute("UPDATE permissions SET users = ? WHERE permission_name = ?", (users, permission_name))
        self.connection.commit()
    ...
    def add_role(self, permission_name: str, role_id: int) -> None:
        ...
        roles = self.cursor.execute("SELECT roles FROM permissions WHERE permission_name = ?", (permission_name,)).fetchone()
        if roles is None:
            roles = []
        else:
            roles = roles[0].split(",")
        ...
        roles = ",".join(roles)
        self.cursor.execute("UPDATE permissions SET roles = ? WHERE permission_name = ?", (roles, permission_name))
        self.connection.commit()
    ...
    def contains_permission(self, permission_name: str, user: discord.Member) -> bool:
        ...
        users = self.cursor.execute("SELECT users FROM permissions WHERE permission_name = ?", ("*.admin",)).fetchone()
        ...
        roles = self.cursor.execute("SELECT roles FROM permissions WHERE permission_name = ?", (permission_name,)).fetchone()[0]
    ...
    def list_permissions(self) -> list[str]:
        return [x[0] for x in self.cursor.execute("SELECT permission_name FROM permissions").fetchall()]
